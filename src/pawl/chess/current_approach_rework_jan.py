...
def draw_for_non_prime(width, height, extra_blocks):  # 3,3,7
    ...
    if difference > 0:
        for _ in range(height):
            string += "G"
        for _ in range(width+extra_rows):
            string += "P"  # WIDTH + 1
        ...
        if extra_rows_remainder > 0:
            for _ in range(1):
                string += "D"  # IF NO REMAINDER THEN ITS NOT NEEDED
            for _ in range(extra_rows_remainder):
                string += "L"
            for _ in range(1):
                string += "G"
            for _ in range(width - extra_rows_remainder):
                string += "L"
        else:
